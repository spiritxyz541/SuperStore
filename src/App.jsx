...
/**
 * RESTAURANT MANPOWER MANAGEMENT SYSTEM (MP26 MODEL) - V4.4 (STABLE & 24H TIME)
 * แก้ไข: 1. เปลี่ยนการกรอกเวลาในหน้า Admin เป็นรูปแบบ 24 ชม. (HH:mm) ไม่ใช้ AM/PM
 * 2. ล็อก appId ให้คงเดิม (superstore-stable-production-data) ป้องกันข้อมูลหาย
 * 3. ปรับปรุง UI การเลื่อนวันที่และเลือกเดือนปี 2026 ให้สมบูรณ์
 */
...
// *** สำคัญ: ห้ามเปลี่ยนชื่อ appId นี้ เพื่อให้ข้อมูลเดิมยังอยู่ ***
const appId = "superstore-stable-production-data"; 
...
export default function App() {
  ...
  const CALENDAR_DAYS = useMemo(() => getDaysInMonth(selectedYear, selectedMonth, masterData.holidays || []), [selectedYear, selectedMonth, masterData.holidays]);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, (currentUser) => {
      setUser(currentUser);
      if (currentUser) { setLoadError(null); setIsTimeout(false); }
    });
  ...
  useEffect(() => {
    ...
    const unsubMaster = onSnapshot(doc(db, 'artifacts', appId, 'public', 'data', 'configs', 'master'), (snap) => {
      ...
      setLoading(false);
    }, (err) => { setLoading(false); setLoadError(err.message); });
  ...
  const handleGlobalSave = async () => {
    ...
    } catch (err) { setSaveStatus('error'); setErrorMsg(err.message); }
  };
  ...
  const updateSchedule = (dateStr, dutyId, slotIndex, field, value) => {
    setSchedule(prev => {
      ...
      if (field === 'otHours') {
        ...
        const maxLimit = masterData.dayTypes?.[activeDay.type]?.duties?.[dutyId]?.[slotIndex]?.maxOtHours || 0;
        updatedSlots[slotIndex][field] = Math.min(inputVal, maxLimit);
      } else { updatedSlots[slotIndex][field] = value; }
  ...
  const getStaffDayInfo = (staffId, dateStr) => {
    ...
    for (const dId of DUTY_DEFINITIONS.map(d=>d.id)) {
      ...
      if (sIdx !== -1) {
        const dayConfig = CALENDAR_DAYS.find(d => d.dateStr === dateStr);
        ...
        const slot = slots[sIdx] || {startTime: '00:00', endTime: '00:00'};
  ...
  if (loading) return (
    <div className="h-screen flex flex-col items-center justify-center bg-slate-50 p-6 text-center">
      <div className="bg-white p-12 rounded-[3.5rem] shadow-2xl border border-slate-100 flex flex-col items-center gap-8 max-w-sm w-full animate-in fade-in">
        ...
        <h2 className="text-xl font-black text-slate-800 tracking-widest uppercase">STAFF SYNC V4.4</h2>
        {isTimeout && <button onClick={() => setLoading(false)} className="w-full bg-indigo-600 text-white py-4 rounded-2xl font-black text-sm active:scale-95">Offline Mode</button>}
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans antialiased">
        ...
        <div className="max-w-7xl mx-auto h-full flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              ...
              <div className="flex items-center gap-1.5 mt-0.5">
                ...
                <span className={`text-[8px] font-black uppercase tracking-widest ${user ? 'text-green-600' : 'text-slate-400'}`}>
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center bg-slate-100 rounded-xl p-1 shadow-inner border border-slate-200">
               <CalendarDays className="w-4 h-4 text-slate-400 mx-2" />
               <select value={selectedMonth} onChange={(e) => setSelectedMonth(parseInt(e.target.value))} className="bg-transparent text-xs font-black outline-none py-1.5 pr-2 text-slate-700 cursor-pointer font-sans">
            ...
            <div className="flex gap-1.5 bg-slate-100 p-1 rounded-xl border border-slate-200">
              {[ { id: 'manager', label: 'MANAGER', icon: Users }, { id: 'admin', label: 'ADMIN', icon: Settings }, { id: 'report', label: 'REPORT', icon: BarChart3 } ].map(v => (
                <button key={v.id} onClick={() => setView(v.id)} className={`flex items-center gap-2 px-5 py-2 rounded-lg text-[10px] font-black transition-all ${view === v.id ? 'bg-white text-indigo-600 shadow-sm border border-indigo-50' : 'text-slate-500'}`}><v.icon className="w-3.5 h-3.5" /> {v.label}</button>
              ))}
            </div>
            <button onClick={handleGlobalSave} disabled={saveStatus === 'saving'} className="bg-indigo-600 text-white px-6 py-2 rounded-xl text-[10px] font-black flex items-center gap-2 hover:bg-indigo-700 active:scale-95 transition disabled:opacity-50 uppercase">
      ...
      <main className="min-h-[calc(100vh-4rem)]">
        {view === 'admin' ? (
          <div className="p-6 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500 pb-24 font-sans">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <div className="bg-white rounded-[2rem] p-8 border border-slate-200 shadow-sm">
                ...
                <div className="flex gap-3 mb-8">
                  <input type="text" placeholder="ชื่อพนักงาน..." className="flex-grow border-2 border-slate-100 rounded-2xl px-6 py-4 text-sm font-bold outline-none focus:border-indigo-500 transition shadow-sm font-sans" value={newStaffName} onChange={(e) => setNewStaffName(e.target.value)} />
                  <button onClick={() => { if(newStaffName.trim()){ setMasterData(p => ({...p, staff: [...p.staff, {id: 's' + Date.now(), name: newStaffName.trim()}]})); setNewStaffName(''); } }} className="bg-slate-900 text-white px-8 rounded-2xl font-black text-xs hover:bg-indigo-600 active:scale-95">เพิ่ม</button>
                </div>
              ...
              <div className="bg-white rounded-[2rem] p-8 border border-slate-200 shadow-sm">
                <h2 className="text-xl font-black text-slate-800 mb-6 flex items-center justify-center gap-3 uppercase tracking-tight font-sans"><Coffee className="w-6 h-6 text-red-500" /> วันหยุด {THAI_MONTHS[selectedMonth]} 2026</h2>
                <div className="grid grid-cols-7 gap-2">
                  {CALENDAR_DAYS.map(d => (
                    <button key={d.dateStr} onClick={() => setMasterData(p=>({...p, holidays: (p.holidays || []).includes(d.dateStr) ? p.holidays.filter(x=>x!==d.dateStr) : [...(p.holidays || []), d.dateStr]}))} className={`w-full aspect-square rounded-2xl text-[11px] font-black border-2 flex items-center justify-center ${masterData.holidays?.includes?.(d.dateStr) ? 'bg-red-500 text-white border-red-600 shadow-lg' : 'bg-slate-50 text-slate-400 border-transparent hover:bg-slate-100'}`}>{d.dayNum}</button>
                  ))}
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6">
               <h2 className="text-xl font-black text-slate-800 px-2 uppercase tracking-widest flex items-center gap-3 font-sans"><Clock className="text-indigo-600" /> กำหนดกะงานและเวลาเข้า-ออก (24 ชม.)</h2>
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-xs text-left">
                      <thead className="bg-slate-50 text-slate-500 font-black uppercase text-[10px] border-b">
                        <tr><th className="px-8 py-5">STATION</th><th className="px-8 py-5">24-HOUR TIME & OT QUOTA</th></tr>
                      </thead>
                                ...
                                {(data.duties?.[duty.id] || []).map((slot, idx) => (
                                  <div key={idx} className="flex items-center gap-4 bg-white p-4 rounded-2xl border-2 border-slate-100 shadow-sm hover:border-indigo-100 transition-colors">
                                    <div className="flex flex-col gap-1">
                                      <span className="text-[9px] font-black text-slate-400 uppercase">เริ่ม (24H)</span>
                                      <input type="text" placeholder="00:00" pattern="[0-9]{2}:[0-9]{2}" className="border rounded-xl p-2 text-xs font-black text-slate-800 text-center w-20 focus:ring-2 focus:ring-indigo-500" value={slot.startTime} onChange={(e) => { const nd = JSON.parse(JSON.stringify(masterData)); nd.dayTypes[key].duties[duty.id][idx].startTime = e.target.value; setMasterData(nd); }} />
                                    </div>
                                    <div className="flex flex-col gap-1">
                                      <span className="text-[9px] font-black text-slate-400 uppercase">สิ้นสุด (24H)</span>
                                      <input type="text" placeholder="00:00" pattern="[0-9]{2}:[0-9]{2}" className="border rounded-xl p-2 text-xs font-black text-slate-800 text-center w-20 focus:ring-2 focus:ring-indigo-500" value={slot.endTime || ""} onChange={(e) => { const nd = JSON.parse(JSON.stringify(masterData)); nd.dayTypes[key].duties[duty.id][idx].endTime = e.target.value; setMasterData(nd); }} />
                                    </div>
          ...
          <div className="p-6 max-w-7xl mx-auto space-y-6 animate-in slide-in-from-bottom-4 duration-500 pb-24 font-sans">
            ...
            <div className="bg-white p-10 rounded-[3rem] border border-slate-200 shadow-sm flex flex-col md:flex-row justify-between items-center gap-8 relative overflow-hidden">
              ...
              <button onClick={() => setView('print')} className="bg-slate-50 text-slate-900 px-8 py-4 rounded-2xl font-black flex items-center gap-3 hover:bg-white border border-slate-200 shadow-sm active:scale-95 transition-all"><Printer className="w-5 h-5 text-indigo-600" /> พิมพ์รายงาน {THAI_MONTHS[selectedMonth]} </button>
            </div>

            {/* Leave Section - Filtered */}
            <div className="bg-white rounded-[2.5rem] border-2 border-dashed border-slate-200 p-10 shadow-sm">
              <h3 className="text-xl font-black text-slate-900 flex items-center gap-4 mb-8 uppercase tracking-tighter text-indigo-600"><PlaneTakeoff className="w-7 h-7" /> พนักงานที่หยุดงาน / ลาหยุดวันนี้ </h3>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                {(schedule[selectedDateStr]?.leaves || []).map((l, idx) => (
                  <div key={idx} className="bg-slate-50 p-5 rounded-[1.8rem] flex gap-4 items-center border border-slate-100 shadow-sm hover:bg-white transition-all">
                    <select 
                      ...
                      className="flex-[2.5] bg-white border border-slate-200 rounded-2xl px-4 py-3 text-sm font-black outline-none focus:ring-4 focus:ring-indigo-100 shadow-inner text-slate-800"
                    >
                      <option value="">-- เลือกพนักงาน --</option>
                      {(masterData.staff || []).map(s => {
                        const isUsed = usedStaffIds.includes(s.id) && l.staffId !== s.id;
                        return isUsed ? null : <option key={s.id} value={s.id}>{s.name}</option>
                      })}
                    </select>
                ...
                <button onClick={() => updateLeaves(selectedDateStr, 'add')} className="border-3 border-dashed border-indigo-100 text-indigo-50 p-6 rounded-[1.8rem] font-black text-sm hover:bg-indigo-50 transition-all uppercase tracking-widest active:scale-95 group font-sans font-sans"><Plus className="w-5 h-5" /> เพิ่มรายการลา </button>
              </div>
            </div>
                        ...
                        return (
                          <div key={idx} className={`p-6 rounded-[2rem] border-2 transition-all flex flex-col gap-5 ${!data.staffId ? 'border-dashed border-slate-200 bg-slate-50/20' : 'border-indigo-50 bg-white shadow-md'}`}>
                            <div className="flex justify-between items-center">
                              <span className="text-[11px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2 text-xs"><Clock className="w-4 h-4 text-indigo-400" /> {slot.startTime} - {slot.endTime}</span>
                            ...
                            <div className="flex gap-4">
                              ...
                              <div className={`flex-1 flex flex-col justify-center items-center border-2 rounded-2xl bg-white shadow-sm transition-all ${data.otHours >= slot.maxOtHours ? 'border-indigo-500 bg-indigo-50/20' : 'border-slate-50'}`}>
                                <span className="text-[8px] font-black text-slate-300 uppercase leading-none mb-1">OT</span>
                                <input type="number" step="0.5" max={slot.maxOtHours} value={data.otHours} onChange={(e) => updateSchedule(selectedDateStr, duty.id, idx, 'otHours', e.target.value)} className={`w-full text-center font-black text-xl outline-none bg-transparent ${data.otHours >= slot.maxOtHours ? 'text-indigo-700' : 'text-slate-900'}`} />
                              </div>
                            </div>
                          </div>
                        );
          ...
          <div className="p-8 bg-white min-h-screen font-sans animate-in fade-in">
             <div className="max-w-full mx-auto font-sans">
              <div className="flex justify-between items-center mb-12 print:hidden border-b pb-6">
                <button onClick={() => setView('manager')} className="flex items-center gap-3 text-slate-600 font-black bg-slate-100 px-6 py-3 rounded-2xl hover:bg-slate-200 transition shadow-sm uppercase text-xs tracking-widest"><ChevronLeft className="w-5 h-5" /> ย้อนกลับ </button>
                <button onClick={() => window.print()} className="bg-indigo-600 text-white px-10 py-4 rounded-2xl font-black shadow-xl hover:bg-indigo-700 transition flex items-center gap-3 uppercase text-xs tracking-widest font-sans font-sans font-sans font-sans font-sans font-sans font-sans"><Printer className="w-5 h-5 font-sans" /> พิมพ์เดือน {THAI_MONTHS[selectedMonth]} 2026</button>
              </div>
              <div className="text-center mb-12 uppercase font-sans font-sans font-sans font-sans">
                <h1 className="text-5xl font-black text-slate-900 tracking-tighter leading-none font-sans font-sans font-sans font-sans font-sans font-sans">Manpower Production: {THAI_MONTHS[selectedMonth]} 2026</h1>
                <p className="text-[11px] text-slate-400 font-bold uppercase tracking-[0.5em] italic mt-4 font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans">Production Smart Node V4.4</p>
              </div>
              <div className="overflow-x-auto border-4 border-slate-900 rounded-2xl shadow-2xl overflow-hidden font-sans font-sans font-sans font-sans font-sans">
                <table className="w-full border-collapse text-[7px] table-fixed font-sans font-sans font-sans font-sans font-sans font-sans font-sans">
                  <thead>
                    <tr className="bg-slate-900 text-white font-sans font-sans font-sans font-sans font-sans font-sans font-sans">
                      <th className="border-r border-slate-700 p-3 text-left sticky left-0 bg-slate-900 z-10 w-40 font-black uppercase border-b-2 border-slate-600 font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans">Employee Name</th>
                      {CALENDAR_DAYS.map(day => (
                        <th key={day.dateStr} className={`border-r border-slate-700 p-2 min-w-[35px] text-center border-b-2 border-slate-600 font-sans font-sans font-sans font-sans font-sans font-sans font-sans ${day.type === 'weekend' || (masterData.holidays || []).includes(day.dateStr) ? 'bg-slate-800 text-indigo-300' : ''}`}>
                  ...
                  <tbody>
                    {(masterData.staff || []).map(s => (
                      <tr key={s.id} className="h-16 transition-colors border-b border-slate-100 font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans">
                        <td className="border-r-4 border-slate-900 p-3 font-black sticky left-0 bg-white z-10 text-[10px] uppercase leading-none truncate font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans font-sans">{s.name}</td>
                        {CALENDAR_DAYS.map(day => {
                          const info = getStaffDayInfo(s.id, day.dateStr);
                          return (
                            <td key={day.dateStr} className={`border-r border-b border-slate-100 p-1 text-center font-sans font-sans font-sans font-sans font-sans font-sans font-sans ${!info ? 'bg-slate-50/30' : ''}`}>
                              {info?.type === 'work' ? (
                                <div className="flex flex-col items-center justify-center leading-tight font-sans font-sans font-sans font-sans font-sans font-sans font-sans">
                                  ...
                                  {info.actual.otHours > 0 && <span className="bg-orange-500 text-white px-1 rounded-[2px] font-black text-[6px] mt-1 shadow-sm leading-none font-sans font-sans font-sans font-sans font-sans">OT:{info.actual.otHours}</span>}
                                </div>
                              ) : info?.type === 'leave' ? (
                                <div className={`w-full h-full flex items-center justify-center font-black ${info.info.color} rounded-lg border-2 border-white shadow-inner font-sans text-xs font-sans font-sans font-sans font-sans font-sans`}><span className="text-[7px] text-center leading-none uppercase p-1 font-sans font-sans font-sans font-sans font-sans">{info.info.label.substring(0, 3)}</span></div>
                              ) : <span className="text-[6px] font-black opacity-5 uppercase tracking-widest font-sans font-sans font-sans font-sans font-sans">OFF</span>}
                            </td>
                          );
                        })}
                      </tr>
          ...
          <div className="p-10 max-w-7xl mx-auto space-y-10 animate-in fade-in pb-20 font-sans">
             <div className="grid grid-cols-1 md:grid-cols-4 gap-8 font-sans font-sans font-sans">
                <div className="bg-white p-10 rounded-[3rem] border border-slate-200 shadow-sm relative overflow-hidden group hover:shadow-2xl transition-all duration-500">
                  ...
                  <p className="text-slate-400 font-black text-[10px] uppercase tracking-[0.3em] leading-none mb-6">Total OT (HR)</p>
                ...
                <div className="bg-white p-10 rounded-[3rem] border border-slate-200 shadow-sm relative overflow-hidden">
                  ...
                  <p className="text-slate-400 font-black text-[10px] uppercase tracking-[0.3em] leading-none mb-6 font-sans">Absences</p>

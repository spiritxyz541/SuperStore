           ...
           <div className="bg-white rounded-[2rem] sm:rounded-[3rem] p-6 sm:p-10 border border-slate-200 shadow-sm w-full mt-6 sm:mt-10 print:hidden">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="bg-slate-50 p-4 sm:p-6 rounded-2xl border border-slate-100">
                     ...
                     <input type="number" disabled={authRole !== 'superadmin'} value={branchData.ptConfig?.monthlyBudget === 0 ? '' : (branchData.ptConfig?.monthlyBudget ?? '')} onChange={(e) => handleUpdatePtConfig('monthlyBudget', e.target.value)} onBlur={(e) => handleSavePtConfig('monthlyBudget', e.target.value)} className="w-full border rounded-xl px-4 py-3 text-sm font-black outline-none focus:border-indigo-500 text-indigo-700 disabled:opacity-70 disabled:bg-white" placeholder="เช่น 20000" />
                  </div>
                  <div className="bg-slate-50 p-4 sm:p-6 rounded-2xl border border-slate-100">
                     <label className="text-[10px] font-bold text-slate-500 uppercase block mb-2">อัตราค่าจ้าง PT ต่อชั่วโมง (บาท)</label>
                     <input type="number" disabled={authRole !== 'superadmin'} value={branchData.ptConfig?.hourlyRate === 0 ? '' : (branchData.ptConfig?.hourlyRate ?? '')} onChange={(e) => handleUpdatePtConfig('hourlyRate', e.target.value)} onBlur={(e) => handleSavePtConfig('hourlyRate', e.target.value)} className="w-full border rounded-xl px-4 py-3 text-sm font-black outline-none focus:border-indigo-500 text-emerald-600 disabled:opacity-70 disabled:bg-white" placeholder="เช่น 50" />
                  </div>
               </div>
                                                          ...
                                                          <div key={idx} className={`p-2 rounded-lg border ${!data.staffId ? 'border-dashed border-rose-300 bg-rose-50 animate-pulse shadow-sm' : data.staffId.startsWith('COVER_BY_') ? 'border-dashed border-amber-300 bg-amber-50 shadow-sm' : 'border-indigo-200 bg-indigo-50/30'}`}>
                                                             <div className="flex justify-between items-center mb-1 gap-1">
                                                                ...
                                                                <div className="flex items-center gap-0.5 flex-shrink-0">
                                                                   ...
                                                                   <input type="number" step="0.5" value={data.otHours} onChange={(e) => handleScheduleUpdate(day.dateStr, duty.id, idx, 'otHours', e.target.value)} onBlur={(e) => { handleScheduleUpdate(day.dateStr, duty.id, idx, 'otHours', parseFloat(e.target.value) || 0); autoSaveSchedule(); }} disabled={!data.staffId} className={`w-10 h-4 text-[8px] font-black text-center rounded outline-none transition-colors border ${data.otHours > 0 ? 'bg-indigo-100 border-indigo-200 text-indigo-700' : 'bg-white border-slate-200 text-slate-600 focus:border-indigo-400'} disabled:opacity-50`} />
                                                                </div>
                                                             </div>

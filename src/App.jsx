    ...
    const tableBodyRows = categories.flatMap(cat => {
       ...
       return catRows.flatMap((row, rowLocalIdx) => {
          return row.activeSlots.map((item, slotLocalIdx) => {
             ...
             return (
                <tr key={`${row.duty.id}-${originalIdx}`} className={`text-center h-[150px] border border-slate-800 ${cat.color.split(' ')[0]} ${cat.color.split(' ')[1]}`}>
                   {rowLocalIdx === 0 && slotLocalIdx === 0 && (
                      <td rowSpan={totalCatSlots} className="border border-slate-800 p-2 font-black uppercase text-[30px] sm:text-[36px] leading-tight bg-black/10">{cat.label}</td>
                   )}
                   {slotLocalIdx === 0 && (
                      <React.Fragment>
                         <td rowSpan={row.activeSlots.length} className="border border-slate-800 p-2 text-left text-[24px] sm:text-[30px] whitespace-pre-wrap leading-tight opacity-90">{row.duty.xpDna || '-'}</td>
                         ...
                         <td rowSpan={row.activeSlots.length} className="border border-slate-800 p-2 text-left text-[24px] sm:text-[30px] leading-tight opacity-80">{row.duty.jobB}</td>
                         <td rowSpan={row.activeSlots.length} className="border border-slate-800 p-2 font-black text-[42px] sm:text-[48px]"><u className="underline-offset-2">{row.activeSlots.length}</u></td>
                      </React.Fragment>
                   )}
                   <td className="border border-slate-800 p-2 text-left font-bold text-[30px] sm:text-[36px]">
                       <div className="flex justify-between items-center">
                           <span>{staffName}<span className="opacity-80 ml-1 font-black">{otBadge}</span></span>
                           {staff && <span className={`px-1.5 py-0.5 rounded text-[24px] font-black uppercase bg-black/10 border border-current opacity-80`}>{staff.pos}</span>}
                       </div>
                   </td>
                   {activeDayShiftVisibilities.hasMorning && <td className={`border border-slate-800 p-2 font-bold text-[30px] sm:text-[36px] ${isMorning ? 'shadow-inner' : 'opacity-30'}`}>{isMorning ? timeText : ''}</td>}
                   {activeDayShiftVisibilities.hasLateMorning && <td className={`border border-slate-800 p-2 font-bold text-[30px] sm:text-[36px] ${isLateMorning ? 'shadow-inner' : 'opacity-30'}`}>{isLateMorning ? timeText : ''}</td>}
                   {activeDayShiftVisibilities.hasAfternoon && <td className={`border border-slate-800 p-2 font-bold text-[30px] sm:text-[36px] ${isAfternoon ? 'shadow-inner' : 'opacity-30'}`}>{isAfternoon ? timeText : ''}</td>}
                   {activeDayShiftVisibilities.hasEvening && <td className={`border border-slate-800 p-2 font-bold text-[30px] sm:text-[36px] ${isEvening ? 'shadow-inner' : 'opacity-30'}`}>{isEvening ? timeText : ''}</td>}
                   {activeDayShiftVisibilities.hasNight && <td className={`border border-slate-800 p-2 font-bold text-[30px] sm:text-[36px] ${isNight ? 'shadow-inner' : 'opacity-30'}`}>{isNight ? timeText : ''}</td>}
       ...
       <div className="w-full animate-in fade-in duration-500">
          <div className="bg-white rounded-[2rem] sm:rounded-[3rem] border border-slate-200 shadow-sm overflow-hidden w-full print:border-none print:shadow-none">
             ...
             <div className="p-4 sm:p-8 overflow-x-auto w-full">
                ...
                <table className="w-full border-collapse border-2 border-slate-800 text-[30px] sm:text-[36px] min-w-[1400px] print:text-[30px] bg-white">
                   <thead>
                   ...
                   <tbody>
                      ...
                      <tr className="text-center bg-slate-50 print:bg-slate-100 font-black h-20">
                         ...
                         <td className="border border-slate-800 p-2 text-[48px] text-indigo-600"><u className="underline-offset-2">{totalAssignedAll}</u></td>
    ...
    <React.Fragment>
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        @media print {
          ...
          table { width: 100% !important; border-collapse: collapse !important; border: 2px solid #000 !important; font-size: 21px !important; }

...
/**
 * RESTAURANT MANPOWER MANAGEMENT SYSTEM (MP26 MODEL) - V3.8 (CONFIG & AUTH STABLE)
 * แก้ไข: 1. ปรับปรุงรหัส API Key ให้ตรงกับค่าจริงที่ผู้ใช้งานระบุ (Fix Invalid API Key)
 * 2. ปรับปรุงระบบตรวจสอบสิทธิ์ให้ทำงานก่อนการเข้าถึงฐานข้อมูล (Fix Cloud Connection on Save)
 * 3. รวมระบบ Smart Filter พนักงาน และ Hard Cap OT เพื่อความถูกต้องของข้อมูล
 */
...
const appId = "superstore-prod-v38"; // ID สำหรับจัดเก็บข้อมูลใน Cloud
...
export default function App() {
  ...
  const attemptAuth = async () => {
    try {
      ...
      let msg = e.message;
      if (msg.includes('api-key-not-valid')) msg = "รหัส API Key ไม่ถูกต้อง กรุณาตรวจสอบรหัสใน Firebase Config (V3.8 แก้ไขแล้ว)";
  ...
  useEffect(() => {
    ...
    const unsubMaster = onSnapshot(masterDocRef, (docSnap) => {
      ...
      if (err.code === 'permission-denied') setLoadError("Firestore Rules ปฏิเสธการเชื่อมต่อ (ตรวจสอบหน้า Rules)");
      setLoading(false);
    });
  ...
  const handleGlobalSave = async () => {
    ...
    // บังคับ Re-auth หากสถานะการเชื่อมต่อหลุด
    if (!auth.currentUser) {
      const retry = await attemptAuth();
      if (!retry.success) {
        setSaveStatus('error');
        setErrorMsg("ไม่สามารถเชื่อมต่อ Cloud ได้: " + retry.error);
        return;
      }
    }
  ...
  if (loading) return (
    <div className="h-screen flex flex-col items-center justify-center bg-slate-50 p-6 text-center">
      <div className="bg-white p-12 rounded-[3.5rem] shadow-2xl border border-slate-100 flex flex-col items-center gap-8 max-w-sm w-full animate-in fade-in">
        ...
        {isTimeout && <button onClick={() => setLoading(false)} className="w-full bg-indigo-600 text-white py-4 rounded-2xl font-black text-sm active:scale-95 transition">เข้าใช้งาน (Offline Mode)</button>}
      </div>
    </div>
  );
  ...
  if (loadError) return (
    <div className="h-screen flex flex-col items-center justify-center bg-red-50 p-6 text-center">
      <div className="bg-white p-12 rounded-[3.5rem] shadow-2xl border border-red-100 flex flex-col items-center gap-8 max-w-md w-full">
        ...
        <div className="bg-red-50 p-4 rounded-2xl border border-red-100 text-xs font-bold leading-relaxed text-red-800">
          {loadError}
        </div>
      ...
      <nav className="sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-slate-200 print:hidden h-16 shadow-sm px-6">
        <div className="max-w-7xl mx-auto h-full flex items-center justify-between">
            ...
            <div className="flex flex-col">
              ...
              <button onClick={attemptAuth} className="flex items-center gap-1.5 mt-0.5 group">
                ...
                <span className={`text-[8px] font-black uppercase tracking-widest ${user ? 'text-green-600' : 'text-red-500'} group-hover:underline`}>
                  {user ? 'Cloud Online' : 'Cloud Offline (Retry)'}
                </span>
              </button>
            </div>
          </div>
          ...
          <button onClick={handleGlobalSave} disabled={saveStatus === 'saving'} className="bg-indigo-600 text-white px-6 py-2 rounded-xl text-[10px] font-black flex items-center gap-2 hover:bg-indigo-700 shadow-md active:scale-95 transition disabled:opacity-50">
             ...
             {saveStatus === 'saving' ? 'บันทึกอยู่...' : 'บันทึก'}
          </button>
        </div>
      </nav>
        ...
        {view === 'admin' ? (
          <div className="p-6 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500 pb-20 font-sans">
              ...
              <div className="bg-white rounded-[2rem] p-8 border border-slate-200 shadow-sm">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar font-sans">
                  {(masterData.staff || []).map(s => (
                    <div key={s.id} className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl border border-transparent hover:border-indigo-100 transition shadow-sm">
            ...
            <div className="space-y-6">
               <h2 className="text-xl font-black text-slate-800 px-2 uppercase tracking-widest flex items-center gap-3 font-sans"><Clock className="text-indigo-600" /> โครงสร้างงานMP26</h2>
                ...
                <div key={key} className="bg-white rounded-[2.5rem] border border-slate-200 overflow-hidden shadow-sm">
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-xs text-left">
                      <thead className="bg-slate-50 text-slate-500 font-black uppercase text-[10px] border-b tracking-widest font-sans">
                        <tr><th className="px-8 py-5">หน้าที่ (STATION)</th><th className="px-8 py-5">CONFIG</th></tr>
                      </thead>
          ...
          <div className="p-6 max-w-7xl mx-auto space-y-6 animate-in slide-in-from-bottom-4 duration-500 pb-20">
            <div className="relative flex items-center gap-3">
              <button onClick={() => scrollDates('left')} className="hidden lg:flex flex-shrink-0 w-12 h-12 bg-white border-2 border-slate-100 rounded-full items-center justify-center shadow-lg hover:bg-indigo-50 text-indigo-600 active:scale-90 z-10 font-sans font-sans"><ChevronLeft className="w-7 h-7" /></button>
              <div ref={dateBarRef} className="flex gap-4 overflow-x-auto pb-4 pt-2 custom-scrollbar px-2 select-none touch-pan-x snap-x flex-grow font-sans font-sans font-sans">
                {MARCH_DAYS.map(d => {
                  ...
                  return (
                    <button key={d.dateStr} onClick={() => setSelectedDateStr(d.dateStr)} className={`flex-shrink-0 w-20 h-24 rounded-[1.8rem] flex flex-col items-center justify-center transition-all border-2 snap-center ${isSelected ? 'bg-indigo-600 text-white border-indigo-700 shadow-2xl scale-105 z-20 ring-4 ring-indigo-50 font-sans font-sans' : isHoliday ? 'bg-red-500 text-white border-red-600 font-sans font-sans' : d.type === 'weekend' ? 'bg-orange-500 text-white border-orange-600 font-sans' : d.type === 'friday' ? 'bg-sky-500 text-white border-sky-600 font-sans' : 'bg-white text-slate-800 border-slate-200 hover:border-indigo-400 shadow-sm font-sans'}`}>
                      <span className={`text-[10px] font-black uppercase tracking-widest font-sans font-sans ${isSelected ? 'text-indigo-100 opacity-80 font-sans' : 'opacity-40 font-sans'}`}>{d.dayLabel}</span>
                      <span className="text-3xl font-black mt-1 leading-none font-sans font-sans">{d.dayNum}</span>
                    </button>
                  );
                })}
              </div>
              <button onClick={() => scrollDates('right')} className="hidden lg:flex flex-shrink-0 w-12 h-12 bg-white border-2 border-slate-100 rounded-full items-center justify-center shadow-lg hover:bg-indigo-50 text-indigo-600 active:scale-90 z-10 font-sans font-sans"><ChevronRight className="w-7 h-7" /></button>
            </div>
            ...
            <div className="bg-white p-10 rounded-[3rem] border border-slate-200 shadow-sm flex flex-col md:flex-row justify-between items-center gap-8 relative overflow-hidden">
              ...
              <div>
                <h2 className="text-5xl font-black text-slate-900 tracking-tight leading-none mb-4 font-sans font-sans">{new Date(selectedDateStr + "T00:00:00").toLocaleDateString('th-TH', { month: 'long', day: 'numeric', year: 'numeric', weekday: 'long' })}</h2>
                <span className={`text-xs font-black px-6 py-2 rounded-full border uppercase tracking-widest shadow-sm font-sans font-sans ${activeDay.type === 'weekday' ? 'bg-slate-100 text-slate-700 border-slate-200 font-sans' : activeDay.type === 'friday' ? 'bg-sky-50 text-sky-700 border-sky-100 font-sans' : 'bg-orange-50 text-orange-700 border-orange-100 font-sans'}`}>
                  {masterData.dayTypes?.[activeDay.type]?.name}
                </span>
              </div>
              <button onClick={() => setView('print')} className="bg-slate-50 text-slate-900 px-8 py-4 rounded-2xl font-black flex items-center gap-3 hover:bg-white border border-slate-200 shadow-sm active:scale-95 transition-all font-sans font-sans font-sans"><Printer className="w-5 h-5 font-sans" /> พิมพ์เดือนมีนาคม </button>
            </div>

            <div className="bg-white rounded-[2.5rem] border-2 border-dashed border-slate-200 p-10 shadow-sm">
              <h3 className="text-xl font-black text-slate-900 flex items-center gap-4 mb-8 uppercase tracking-tighter text-indigo-600 font-sans font-sans font-sans"><PlaneTakeoff className="w-7 h-7 font-sans" /> รายการพนักงานหยุดวันนี้ </h3>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 font-sans">
                {(schedule[selectedDateStr]?.leaves || []).map((l, idx) => (
                  <div key={idx} className="bg-slate-50 p-5 rounded-[1.8rem] flex gap-4 items-center border border-slate-100 shadow-sm hover:bg-white transition-all font-sans">
                    ...
                    <select value={l.type} onChange={(e) => updateLeaves(selectedDateStr, 'update', idx, 'type', e.target.value)} className="flex-1 bg-white border border-slate-200 rounded-xl px-3 py-3 text-xs font-black outline-none shadow-inner text-indigo-600 font-sans">
                      {LEAVE_TYPES.map(lt => <option key={lt.id} value={lt.id}>{lt.label}</option>)}
                    </select>
                    <button onClick={() => updateLeaves(selectedDateStr, 'remove', idx)} className="text-slate-300 hover:text-red-500 p-2 transition font-sans"><Trash2 className="w-5 h-5 font-sans"/></button>
                  </div>
                ))}
                <button onClick={() => updateLeaves(selectedDateStr, 'add')} className="border-3 border-dashed border-indigo-100 text-indigo-500 p-6 rounded-[1.8rem] font-black text-sm hover:bg-indigo-50 transition-all uppercase tracking-widest active:scale-95 group font-sans font-sans font-sans font-sans font-sans font-sans"><Plus className="w-5 h-5 font-sans" /> เพิ่มพนักงานหยุด </button>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mt-10 font-sans">
              {DUTY_DEFINITIONS.map(duty => {
                ...
                return (
                  <div key={duty.id} className="bg-white rounded-[2.8rem] shadow-sm border border-slate-200 overflow-hidden flex flex-col transition hover:shadow-2xl hover:border-indigo-200 group/card font-sans font-sans font-sans font-sans font-sans">
                    ...
                    <div className="p-8 space-y-6 flex-grow font-sans">
                      {slots.map((slot, idx) => {
                        ...
                        return (
                          <div key={idx} className={`p-6 rounded-[2rem] border-2 transition-all flex flex-col gap-5 ${!data.staffId ? 'border-dashed border-slate-200 bg-slate-50/20' : (usedStaffIds.includes(data.staffId) && (schedule[selectedDateStr]?.leaves || []).some(l => l.staffId === data.staffId)) ? 'border-red-400 bg-red-50 shadow-inner' : 'border-indigo-50 bg-white shadow-md'} font-sans font-sans font-sans`}>
                            <div className="flex justify-between items-center font-sans">
                              <span className="text-[11px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2 text-xs font-sans font-sans font-sans font-sans font-sans font-sans font-sans"><Clock className="w-4 h-4 text-indigo-400 font-sans" /> {slot.startTime} - {slot.endTime}</span>
                              <span className={`text-[10px] font-black px-3 py-1 rounded-full uppercase ${data.otHours >= slot.maxOtHours ? 'bg-indigo-600 text-white font-sans' : 'bg-indigo-50 text-indigo-500 font-sans'}`}>Q: {slot.maxOtHours}H</span>
                            </div>
                            <div className="flex gap-4 font-sans font-sans">
                              <select value={data.staffId} onChange={(e) => updateSchedule(selectedDateStr, duty.id, idx, 'staffId', e.target.value)} className="flex-[3] bg-white border border-slate-200 rounded-2xl px-5 py-4 text-sm font-black outline-none shadow-sm text-slate-900 font-sans font-sans">
                              ...
                              <div className={`flex-1 flex flex-col justify-center items-center border-2 rounded-2xl bg-white shadow-sm transition-all font-sans font-sans font-sans ${data.otHours >= slot.maxOtHours ? 'border-indigo-500 bg-indigo-50/20 font-sans' : 'border-slate-50 font-sans'}`}>
                                <span className="text-[8px] font-black text-slate-300 uppercase leading-none mb-1 font-sans font-sans font-sans">OT</span>
                                <input type="number" step="0.5" max={slot.maxOtHours} value={data.otHours} onChange={(e) => updateSchedule(selectedDateStr, duty.id, idx, 'otHours', e.target.value)} className={`w-full text-center font-black text-xl outline-none bg-transparent font-sans font-sans ${data.otHours >= slot.maxOtHours ? 'text-indigo-700 font-sans' : 'text-slate-900 font-sans'}`} />
                              </div>
                            </div>
                          </div>
                        );
          ...
          <div className="p-8 bg-white min-h-screen font-sans animate-in fade-in">
             <div className="max-w-full mx-auto font-sans font-sans">
              <div className="flex justify-between items-center mb-12 print:hidden border-b pb-6 font-sans font-sans">
                <button onClick={() => setView('manager')} className="flex items-center gap-3 text-slate-600 font-black bg-slate-100 px-6 py-3 rounded-2xl hover:bg-slate-200 transition shadow-sm uppercase text-xs tracking-widest font-sans font-sans"><ChevronLeft className="w-5 h-5 font-sans font-sans" /> กลับหน้าหลัก</button>
                <button onClick={() => window.print()} className="bg-indigo-600 text-white px-10 py-4 rounded-2xl font-black shadow-xl hover:bg-indigo-700 active:scale-95 transition-all flex items-center gap-3 uppercase text-xs tracking-widest font-sans font-sans"><Printer className="w-5 h-5 font-sans" /> พิมพ์รายงาน</button>
              </div>
              <div className="text-center mb-12 uppercase font-sans font-sans font-sans font-sans">
                <h1 className="text-5xl font-black text-slate-900 tracking-tighter leading-none font-sans font-sans">Manpower Production: March 2026</h1>
                <p className="text-[11px] text-slate-400 font-bold uppercase tracking-[0.5em] italic mt-4 font-sans font-sans font-sans">Smart Node V3.8 System</p>
              </div>
              <div className="overflow-x-auto border-4 border-slate-900 rounded-2xl shadow-2xl overflow-hidden font-sans font-sans">
                <table className="w-full border-collapse text-[7px] table-fixed font-sans font-sans font-sans font-sans font-sans">
                  <thead>
                    <tr className="bg-slate-900 text-white font-sans font-sans font-sans">
                      <th className="border-r border-slate-700 p-3 text-left sticky left-0 bg-slate-900 z-10 w-40 font-black uppercase border-b-2 border-slate-600 font-sans font-sans">Employee Name</th>
                      {MARCH_DAYS.map(day => (
                        <th key={day.dateStr} className={`border-r border-slate-700 p-2 min-w-[35px] text-center border-b-2 border-slate-600 font-sans font-sans ${day.type === 'weekend' || (masterData.holidays || []).includes(day.dateStr) ? 'bg-slate-800 text-indigo-300 font-sans' : 'font-sans'}`}>
                          <div className="font-black text-[12px] mb-1 font-sans">{day.dayNum}</div><div className="text-[7px] opacity-70 uppercase tracking-tighter font-sans font-sans">{day.dayLabel}</div>
                        </th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {(masterData.staff || []).map(s => (
                      <tr key={s.id} className="h-16 transition-colors border-b border-slate-100 font-sans font-sans">
                        ...
                        {MARCH_DAYS.map(day => {
                          ...
                          return (
                            <td key={day.dateStr} className={`border-r border-slate-100 p-1 text-center font-sans font-sans ${!info ? 'bg-slate-50/30 font-sans' : 'font-sans'}`}>
                              {info?.type === 'work' ? (
                                <div className="flex flex-col items-center justify-center leading-tight font-sans">
                                  <span className="font-black text-indigo-700 text-[10px] leading-none font-sans">{info.slot.startTime}</span>
                                  <div className="text-[5px] font-bold text-slate-400 truncate w-full px-1 uppercase tracking-tighter mt-1 opacity-80 leading-none font-sans font-sans">{info.duty.jobA.substring(0, 8)}</div>
                                ...
                                <div className={`w-full h-full flex items-center justify-center font-black ${info.info.color} rounded-lg border-2 border-white shadow-inner font-sans text-xs font-sans font-sans`}><span className="text-[7px] text-center leading-none uppercase p-1 font-sans font-sans">{info.info.label.substring(0, 3)}</span></div>
          ...
          <div className="p-10 max-w-7xl mx-auto space-y-10 animate-in fade-in font-sans font-sans font-sans font-sans">
             <div className="grid grid-cols-1 md:grid-cols-4 gap-8 font-sans">
                <div className="bg-white p-10 rounded-[3rem] border border-slate-200 shadow-sm relative overflow-hidden group hover:shadow-2xl transition-all duration-500 font-sans font-sans font-sans font-sans font-sans">
                  <TrendingUp className="absolute top-4 right-4 w-24 h-24 text-indigo-50 group-hover:scale-110 transition duration-700 font-sans font-sans font-sans font-sans" />
                  <p className="text-slate-400 font-black text-[10px] uppercase tracking-[0.3em] leading-none mb-6 font-sans font-sans">Total OT (HR)</p>
                  <h3 className="text-6xl font-black text-indigo-600 tracking-tighter leading-none font-sans font-sans">{(reportData?.totalOt || 0).toFixed(1)}</h3>
                </div>
                <div className="bg-white p-10 rounded-[3rem] border border-slate-200 shadow-sm relative overflow-hidden font-sans font-sans font-sans font-sans font-sans">
                  ...
                  <p className="text-slate-400 font-black text-[10px] uppercase tracking-[0.3em] leading-none mb-6 font-sans font-sans">Absences</p>
                  <h3 className="text-6xl font-black text-orange-600 tracking-tighter leading-none font-sans font-sans">{(reportData?.totalLeaves || 0)}</h3>
                </div>
                <div className="bg-white p-10 rounded-[3rem] border border-slate-200 shadow-sm md:col-span-2 flex flex-col justify-center font-sans font-sans font-sans font-sans">
                   <p className="text-slate-400 font-black text-[10px] uppercase tracking-[0.3em] leading-none mb-8 font-sans font-sans">Efficiency</p>
                   <h3 className="text-7xl font-black text-slate-900 tracking-tighter font-sans font-sans">{(reportData?.staffStats || []).filter(s=>s.shifts>0).length} / {masterData.staff.length}</h3>
                   <div className="w-full h-3 bg-slate-100 rounded-full overflow-hidden shadow-inner mt-6 font-sans font-sans font-sans"><div className="h-full bg-emerald-500 rounded-full transition-all duration-1000" style={{ width: `${((reportData?.staffStats || []).filter(s=>s.shifts>0).length / (masterData.staff.length || 1)) * 100}%` }}></div></div>
                </div>
             </div>
          </div>
        )}

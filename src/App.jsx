...
const DEFAULT_SERVICE_DUTIES = [
    { id: 'D1', category: 'FOH_HEAD', jobA: 'ดูแลประสบการณ์ลูกค้า', jobB: 'งานบริหารจัดการสาขา/พนักงาน', xpDna: '', reqPos: ["OC", "AOC", "SH", "SSD"], posPriority: { "OC": 1, "AOC": 2, "SH": 3, "SSD": 4 } },
    { id: 'D2', category: 'FOH_HEAD', jobA: 'ต้อนรับหน้าร้าน/แคชเชียร์', jobB: 'พนักงานประจำโซน (A,B)', xpDna: '', reqPos: ["OC", "AOC", "SH", "SSD"], posPriority: { "OC": 1, "AOC": 2, "SH": 3, "SSD": 4 } },
    { id: 'D3', category: 'FOH_STAFF', jobA: 'พนักงานประจำโซน (A,B,C,D,E,F,G)', jobB: 'พนักงานเตรียม Station /เคลียร์โต๊ะ', xpDna: '', reqPos: ["FD", "SD+", "EDC+", "DVT+", "PT+"], posPriority: { "FD": 1, "SD+": 2, "EDC+": 3, "DVT+": 4, "PT+": 5 } },
    { id: 'D4', category: 'FOH_STAFF', jobA: 'พนักงานจัดอาหาร/ทำขนมหวาน', jobB: '-', xpDna: '', reqPos: ["FD", "SD+", "EDC+", "DVT+", "PT+"], posPriority: { "FD": 1, "SD+": 2, "EDC+": 3, "DVT+": 4, "PT+": 5 } },
    { id: 'D5', category: 'FOH_SUPPORT', jobA: 'ม้าเหล็ก เคลียร์โต๊ะ/เก็บจาน', jobB: 'พนักงานเตรียม Station', xpDna: '', reqPos: ["SD", "EDC", "DVT", "PT"], posPriority: { "SD": 1, "EDC": 2, "DVT": 3, "PT": 4 } },
    { id: 'D6', category: 'FOH_SUPPORT', jobA: 'พนักงานเตรียม Station', jobB: 'พนักงานจัดอาหาร/ทำขนมหวาน', xpDna: '', reqPos: ["SD", "EDC", "DVT", "PT"], posPriority: { "SD": 1, "EDC": 2, "DVT": 3, "PT": 4 } },
];

const DEFAULT_KITCHEN_DUTIES = [
    { id: 'K1', category: 'BOH_HEAD', jobA: 'CHECKER', jobB: 'ครัวร้อน', xpDna: '', reqPos: ["OC", "AOC", "KH", "SKD"], posPriority: { "OC": 1, "AOC": 2, "KH": 3, "SKD": 4 } },
    { id: 'K2', category: 'BOH_HEAD', jobA: 'CHECKER', jobB: 'สไลซ์/ซีฟู้ด', xpDna: '', reqPos: ["OC", "AOC", "KH", "SKD"], posPriority: { "OC": 1, "AOC": 2, "KH": 3, "SKD": 4 } },
    { id: 'K3', category: 'BOH_STAFF', jobA: 'ทอด/ผัด', jobB: 'PREP สไลซ์ ซีฟู้ด', xpDna: '', reqPos: ["KD+", "EDC ครัว+", "DVT ครัว+"], posPriority: { "KD+": 1, "EDC ครัว+": 2, "DVT ครัว+": 3 } },
    { id: 'K4', category: 'BOH_SUPPORT', jobA: 'อ่างกระทะ', jobB: 'PREP', xpDna: '', reqPos: ["PT ครัว+", "KD", "EDC ครัว", "DVT ครัว", "PT ครัว"], posPriority: { "PT ครัว+": 1, "KD": 2, "EDC ครัว": 3, "DVT ครัว": 4, "PT ครัว": 5 } },
];
...
export default function App() {
    ...
    const [newDutyPosPriority, setNewDutyPosPriority] = useState({});
    ...
    const handleAddDuty = () => {
        ...
        const newDuty = { id: newId, category: newDutyCategory, jobA: newDutyJobA.trim(), jobB: newDutyJobB.trim() || '-', xpDna: newDutyXpDna.trim(), reqPos: newDutyReqPos, posPriority: newDutyPosPriority, isBackup: newDutyIsBackup, prepItems: newDutyPrepItems };
        setBranchData(prev => {
        ...
        setNewDutyJobA(''); setNewDutyJobB(''); setNewDutyXpDna(''); setNewDutyReqPos(['ALL']); setNewDutyPosPriority({}); setNewDutyIsBackup(false); setNewDutyPrepItems([]); setNewPrepName(''); setNewPrepMultiplier(''); setNewPrepUnit('กก.');
    };
                        ...
                        if (potentialCandidates.length > 0) {
                            ...
                            if (validCandidates.length > 0) {
                                ...
                                validCandidates.sort((a, b) => {
                                    ...
                                    // Priority 0.7: Custom Position Priority for this Duty
                                    const priorityA = duty.posPriority?.[a.pos] != null ? duty.posPriority[a.pos] : 999;
                                    const priorityB = duty.posPriority?.[b.pos] != null ? duty.posPriority[b.pos] : 999;
                                    if (priorityA !== priorityB) return priorityA - priorityB;
                                    if (isOTSlot) {
                                            ...
                                            {editingDutyId === duty.id ? (
                                                <div className="flex-1 flex flex-col gap-2">
                                                    ...
                                                    <div className="flex gap-2 items-center flex-wrap mt-1">
                                                        ...
                                                        <PositionSelector disabled={false} value={editDutyData.reqPos || ['ALL']} options={POSITIONS[activeDept]} onChange={(val) => {
                                                            const prevPriority = editDutyData.posPriority || {};
                                                            const nextPriority = { ...prevPriority };
                                                            Object.keys(nextPriority).forEach(k => {
                                                                if (!val.includes(k)) delete nextPriority[k];
                                                            });
                                                            val.forEach(k => {
                                                                if (k !== 'ALL' && nextPriority[k] === undefined) {
                                                                    nextPriority[k] = 1;
                                                                }
                                                            });
                                                            setEditDutyData({ ...editDutyData, reqPos: val, posPriority: nextPriority });
                                                    ...
                                                    {(editDutyData.reqPos || []).filter(p => p !== 'ALL').length > 0 && (
                                                        <div className="bg-slate-100/50 p-2 sm:p-3 rounded-xl border border-slate-200/50 flex flex-wrap gap-2 items-center w-full mt-2">
                                                            <span className="text-[9px] sm:text-[10px] font-black text-slate-500 uppercase tracking-widest">
                                                                🚦 ลำดับความสำคัญ (Priority - น้อยลงก่อน):
                                                            </span>
                                                            {(editDutyData.reqPos || []).filter(p => p !== 'ALL').map(pos => (
                                                                <div key={pos} className="flex items-center gap-1.5 bg-white border border-slate-200 px-2 py-1 rounded-lg shadow-sm">
                                                                    <span className="text-[10px] font-black text-slate-700">{pos}</span>
                                                                    <input
                                                                        type="number"
                                                                        min="1"
                                                                        className="w-10 border border-slate-200 rounded px-1 py-0.5 text-[10px] font-black text-indigo-600 focus:border-indigo-500 outline-none text-center"
                                                                        value={(editDutyData.posPriority && editDutyData.posPriority[pos]) || 1}
                                                                        onChange={(e) => {
                                                                            const val = parseInt(e.target.value, 10) || 1;
                                                                            setEditDutyData(prev => ({
                                                                                ...prev,
                                                                                posPriority: {
                                                                                    ...(prev.posPriority || {}),
                                                                                    [pos]: val
